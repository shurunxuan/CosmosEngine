...
void VulkanBackend::Render()
{
    ...
    ubo.model =  mathfu::Quaternion<float>::FromAngleAxis(time * 3.1415927f / 2.0f, mathfu::vec3(0.0f, 0.0f, 1.0f)).ToMatrix4();
    ubo.view = mathfu::mat4::LookAt(mathfu::vec3(0.0f, 0.0f, 0.0f), mathfu::vec3(2.0f, 2.0f, 2.0f), mathfu::vec3(0.0f, 0.0f, 1.0f));
    ubo.proj = mathfu::mat4::Perspective(3.1415927f / 4.0f, swapChainExtent.width / (float) swapChainExtent.height, 0.1f,
                                         10.0f, -1);
    ubo.proj[0] *= -1.0f;
    ubo.proj[5] *= -1.0f;
...
{
    boost::container::vector<VkDescriptorSetLayout> layouts(swapChainImages.size(), descriptorSetLayout);
